  ...
  const loadData = async () => {
    try {
      ...
      addNotification({
        type: 'error',
        title: t('error'),
        message: t('failed_to_load_user_management_data')
      });
    } finally {
      setLoading(false);
    }
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      addNotification({
        ...
        message: t('user_deleted_message', { userName: userToDelete.fullName })
      });
    } catch (error) {
      addNotification({
        type: 'error',
        title: t('delete_failed'),
        message: t('failed_to_delete_user')
      });
    } finally {
    ...
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-gray-600">{t('manage_users_device_assignments')}</p>
        </div>
        <button
          ...
          <table className="w-full">
            <thead className="bg-gray-50 border-b border-gray-200">
              <tr>
                ...
                <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">{t('devices')}</th>
        ...
        {totalPages > 1 && (
          <div className="px-6 py-3 bg-gray-50 border-t border-gray-200 flex items-center justify-between">
            <div className="text-sm text-gray-700">
              {t('showing_x_to_y_of_z_users', { start: startIndex + 1, end: Math.min(endIndex, filteredUsers.length), total: filteredUsers.length })}
            </div>
            <div className="flex space-x-2">
              <button
      ...
      {filteredUsers.length === 0 && !loading && (
        <div className="text-center py-12">
          ...
          <p className="text-gray-500 mb-4">
            {searchQuery || departmentFilter ? t('try_adjusting_filters') : t('create_first_user')}
          </p>
      ...
      {showConfirmDialog && userToDelete && (
        <ConfirmDialog
          ...
          message={t('delete_user_confirm_message', { userName: userToDelete.fullName })}
