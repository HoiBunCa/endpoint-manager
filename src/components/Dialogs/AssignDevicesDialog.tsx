...
const AssignDevicesDialog: React.FC<AssignDevicesDialogProps> = ({ policy, onSave, onClose }) => {
  ...
  const loadDevices = async () => {
    try {
      ...
      addNotification({
        type: 'error',
        title: t('error'),
        message: t('failed_to_load_devices')
      });
    } finally {
      setLoading(false);
    }
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      addNotification({
        type: 'success',
        title: t('devices_assigned'),
        message: t('x_devices_assigned_to_policy', { count: selectedDevices.length, policyName: policy.name })
      });
    } catch (error) {
      addNotification({
        type: 'error',
        title: t('assignment_failed'),
        message: t('failed_to_assign_devices_to_policy')
      });
    } finally {
      setSaving(false);
    }
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white rounded-lg shadow-xl w-full max-w-2xl max-h-[80vh] overflow-hidden">
        <div className="flex items-center justify-between p-6 border-b border-gray-200">
          <div>
            <h2 className="text-xl font-semibold text-gray-900">{t('assign_devices')}</h2>
            <p className="text-sm text-gray-600">{t('select_devices_to_apply_policy', { policyName: policy.name })}</p>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="p-6">
          ...
          <div className="max-h-64 overflow-y-auto border border-gray-200 rounded-lg">
            {loading ? (
              <div className="p-4 text-center text-gray-500">{t('loading_devices')}</div>
            ) : filteredDevices.length === 0 ? (
              <div className="p-4 text-center text-gray-500">{t('no_devices_found')}</div>
            ) : (
                    ...
                    <div className="flex items-center space-x-3">
                      ...
                      <div className="flex-1">
                        ...
                        <div className="text-sm text-gray-600">
                          {device.assignedUser?.fullName || t('unassigned')} • {device.platform}
                        </div>
                      </div>
          ...
          <div className="mt-4 p-3 bg-blue-50 rounded-lg">
            <p className="text-sm text-blue-800">
              {t('x_of_y_devices_selected', { selected: selectedDevices.length, total: devices.length })}
            </p>
          </div>
        </div>
        ...
        <div className="flex justify-end space-x-3 p-6 border-t border-gray-200">
          ...
          >
            {saving ? t('saving') : t('assign_devices')}
          </button>
        </div>
      </div>
    </div>
